...
import { ThyGrid2ViewModel } from './models/thy-grid-view2.class';
...
export abstract class ThyGrid2Typed {
    ...
    @Output() doubleClickChange = new EventEmitter<any>();
    ...
    public dataSource = new MatTableDataSource<ThyGrid2ViewModel<any>>([]);
     ...
     */
    protected viewModels: ThyGrid2ViewModel<any>[] = [];

    /**
     * @private `_selection` store the current selection.
     * This private property is accecible from calculated properties `selection`.
     */
    private _selection: ThyGrid2ViewModel<any> | ThyGrid2ViewModel<any>[] = null;
    public get selection(): ThyGrid2ViewModel<any> | ThyGrid2ViewModel<any>[] { return this._selection instanceof Array ? this._selection.length > 0 ? this._selection : null : this._selection; }
    public set selection(value: ThyGrid2ViewModel<any> | ThyGrid2ViewModel<any>[]) {
      if (this._selection) {
    ...
    public get hasSelection(): boolean { return this.selection instanceof Array ? this.selection.length === 0 ? false : true : !this.selection ? false : true; }
     ...
     */
    public get isThereASelection(): boolean { return this.selection instanceof Array && this.selection.length === 0 ? false : !this.selection ? false : true; }
    ...
    public sort(sort: Sort, a: ThyGrid2ViewModel<any>, b: ThyGrid2ViewModel<any>) {
    ...
    protected filter(vm: ThyGrid2ViewModel<any>, filterValue: string) {
    ...
    public trackById(index: number, item: ThyGrid2ViewModel<any>) {
      return item.id;
    }
    ...
    public select(id: number, type: ThyObjectType) {
      if (id && type && this.dataSource && this.dataSource.data) {
        const selectedElement = this.selection instanceof Array ? this.selection.find(d => d.id === id && d.type === type) : null;
        if (!selectedElement) {
          const element = this.dataSource.data.find(d => d.id === id && d.type === type);
          if (element) {
     ...
     */
    public onSelection(event: any, element: ThyGrid2ViewModel<any>, emitting = false): any {
    ...
    public onContextMenu(event: any, element?: ThyGrid2ViewModel<any> | ThyGrid2ViewModel<any>[], open = true) {
      // if (!this.menu) { return; }
      ...
      if (this.contextMenu) {
        ...
        if (open) {
          this.contextMenu.menuData = { element: this.selection instanceof Array ? this.selection.map(m => m.Model) : this.selection ? this.selection.Model : null };
          this.contextMenu.openMenu();
        }
      }
    }

    public onDoubleClick(element: ThyGrid2ViewModel<any>, ...args: any[]) {
      if (element) {
        this.doubleClickChange.emit(element.Model);
      }
    }
    ...
    public onDrop(event: any, element?: ThyGrid2ViewModel<any>) {
    ...
    public onExport(options?: ThyGridExportOptions) {
      ...
      csv += titles + `\r\n`;
      const list = options.list ? options.list : this.dataSource.data;
