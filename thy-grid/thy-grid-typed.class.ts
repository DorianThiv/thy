...
@Directive()
export abstract class ThyGridTyped<TViewModel extends any> {
     ...
     */
    protected viewModels: TViewModel[] = [];

    /**
     * @private `_selection` store the current selection.
     * This private property is accecible from calculated properties `selection`.
     */
    private _selection: any | any[] = null;
    public get selection(): any | any[] { return this._selection instanceof Array ? this._selection.length > 0 ? this._selection : null : this._selection; }
    public set selection(value: any | any[]) {
      if (this._selection) {
    ...
    public select(id: number, type: ThyObjectType) {
      if (id && type && this.dataSource && this.dataSource.data) {
        const selectedElement = this.selection instanceof Array ? this.selection.find(d => d.Id === id && d.EntityType === type) : null;
        if (!selectedElement) {
    ...
    public isInSelection(element: TViewModel): boolean {
      return (this.selection instanceof Array && this.selection.indexOf(element) !== -1) || (element === this.selection) ? true : false;
    }
    ...
    public onSelection(event: any, element: any, emitting = false): any {
      if (this.disabledSelection) { return true; }
     ...
     * @param event
     */
    public onDragStart(event: DragEvent, element: TViewModel) {
      if (!this.isInSelection(element)) {
        this.selection = [element];
      }
      this._draggingElements = this.selection instanceof Array ? this.selection : [this.selection];
    }
    ...
    protected addToParent(id: number, elements: any[]) {
    }

    public onDrop(event: any, element?: any) {
      event.stopPropagation();
      if (!element) {
        this.addToParent(null, this._draggingElements);
      }
      const height = event.path[0].parentElement.offsetHeight;
      if (event.offsetY <= height * 0.2 || event.offsetY >= height * 0.8) {
        if (!this.isInSelection(element)) {
          this.addToParent(element.ParentId ? element.ParentId : null, this._draggingElements);
        }
      } else {
        if (!this.isInSelection(element)) {
          this.addToParent(element.Id, this._draggingElements);
        }
      }
      setTimeout(() => {
    ...
    @HostListener('window:keyup', ['$event']) onKeydown(event: KeyboardEvent) {
      if (event.keyCode === ThyKeyCode.Escape) {
        if (this._draggingElements && this._draggingElements.length > 0) {
          this.clearDragAndDrop();
        }
      }
    }
